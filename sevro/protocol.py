...
class Sender(Protocol):
    async def write_status(self, status: int = 200, headers: MutableHeaders | None = None): ...

    async def write(self, body: bytes | memoryview | str, more: bool = False): ...

    async def send_file(self, path: str | Path): ...
...
class RSGISender:
    ...
    async def write(self, body: bytes | memoryview | str, more: bool = False):
        if not more and not self._written_headers:
            ...
            if not self._written_headers:
                if self._headers is None or self._status is None:
                    raise ValueError("Headers and status code must be specified before writing")
                self._stream = self.rsgi.response_stream(self._status, self._headers.items())
                self._written_headers = True
            if isinstance(body, str):
    ...
    async def send_file(self, path: str | Path):
        if self._headers is None or self._status is None:
            raise ValueError("Headers and status code must be specified before writing")
        self.rsgi.response_file(self._status, self._headers)
...
class ASGISender:
    ...
    async def send_file(self, path: str | Path):
        if not self._written_headers:
            raise ValueError("Headers must be written first")
        if "extensions" in self.scope and "http.response.pathsend" in self.scope["extensions"]:
            await self.send({
